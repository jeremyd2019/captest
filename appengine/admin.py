...
class UploadSchemaVersion(blobstore_handlers.BlobstoreUploadHandler):
	def post(self):
		...
		dbversion = models.DBVersion.query(models.DBVersion.schema_version == schema_version, models.DBVersion.source_time == st).get() or models.DBVersion()
...
class UploadSchemaForm(webapp2.RequestHandler):
	def get(self):
...
</html>""" % blobstore.create_upload_url('/admin/uploadSchemaVersion', gs_bucket_name=app_identity.get_default_gcs_bucket_name()))

app = webapp2.WSGIApplication([
	('/admin/uploadSchemaVersion', UploadSchemaVersion),
	('/admin/uploadSchemaForm', UploadSchemaForm)
], debug=True)
